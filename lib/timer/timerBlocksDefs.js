        ...
        eventBlockIds.forEach(function(eventBlockId) {
            switch(event.type) {
                case Blockly.Events.BLOCK_CREATE:
                    ...
                    if (["set_interval", "set_timeout"].includes(newBlock.type)) {
                        var newBlockName = newBlock.getField('NAME').getValue();
                        
                        // When a new set_interval/set_timeout block is added (from the flyout) to the workspace, its name will be "interval"/"timeout".
                        // However it should become "interval_x"/"timeout_x" with x the first integer number that is not being used yet by the other 
                        // set_interval/set_timeout blocks in the workspace.                        
                        if (newBlock.type === "set_interval" && newBlockName === Blockly.Msg.TIMER_SET_INTERVAL_NAME) {
                        ...
                        else if (newBlock.type === "set_timeout" && newBlockName === Blockly.Msg.TIMER_SET_TIMEOUT_NAME) {
                            var newUniqueName = getUniqueNameForBlock(Blockly.Msg.TIMER_SET_TIMEOUT_NAME, newBlock);
                            newBlock.setFieldValue(newUniqueName, 'NAME');
                        }

                        // The name of the new set_interval/set_timeout block, should be added to the dropdown options of every clear_interval/clear_timeout block
                    ...
                    else if(["clear_interval", "clear_timeout"].includes(newBlock.type)) {
                        // When a new clear_timout/clear_interval block has been created, all the names (of the existing set_timeout/set_interval blocks) should be added to its dropdown options
                        allBlocks.forEach(function(otherBlock) {
                            if (["set_interval", "set_timeout"].includes(otherBlock.type)) {
                ...
                case Blockly.Events.BLOCK_CHANGE:
                    ...
                    if (["set_interval", "set_timeout"].includes(updatedBlock.type) && event.name === "NAME") {
                        var updatedBlockName = updatedBlock.getField('NAME').getValue();

                        // When the name of a set_timeout/set_interval block has been changed, then that name should also be updated in the dropdown options of every existing clear_timout/clear_interval block
                        allBlocks.forEach(function(otherBlock) {
                            if (["clear_interval", "clear_timeout"].includes(otherBlock.type)) {
                                otherBlock.dropdownOptions[updatedBlock.id] = updatedBlockName;
                                
                                // When the (changed) name of the set_interval/set_timeout was currently selected in the clear_interval/clear_timeout block,
