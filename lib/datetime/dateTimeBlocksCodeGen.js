...
/*
Blockly.JavaScript['datetime_convert_to_date'] = function (block) {
    const input = Blockly.JavaScript.valueToCode(block, "INPUT", Blockly.JavaScript.ORDER_NONE);
    
    var code = 'new Date(' + input + ').getTime()';
    return [code, Blockly.JavaScript.ORDER_ATOMIC];
};
*/
...
Blockly.JavaScript['datetime_convert_from_date'] = function (block) {
    ...
    switch (format) {
        ...
        case 'text':
            ...
            code = 'new Date(' + value + ').toLocaleDateString("' + language + '", {weekday: "short", day: "numeric", month: "short", year: "numeric", hour:"numeric", minute:"numeric"})';
            break;
        case 'full_date':
            code = 'new Date(' + value + ').toISOString().replace(/[^0-9]/g, "")';
            break;
        case 'month_name':
            code = 'new Date(' + value + ').toLocaleDateString("' + language + '", {month: "long"})';
            break;                         
        case 'day_name':
            code = 'new Date(' + value + ').toLocaleDateString("' + language + '", {weekday: "long"})';
            break;             
        case 'dayofweek':
