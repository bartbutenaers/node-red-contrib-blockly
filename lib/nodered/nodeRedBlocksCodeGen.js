...
Blockly.JavaScript['node_send'] = function(block) {
    ...
    // Use a function to make sure we have no magic numbers
    var functionName = Blockly.JavaScript.provideFunction_('isValidOutput',
      [`function ` + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ +
       `(outputNumber) {`,
       `  const AVAILABLE_OUTPUTS = ${Blockly.nodeOutputs};`,
       `  if (outputNumber > AVAILABLE_OUTPUTS) {`,
       `    node.error("Cannot send to output ${outputNumber}, since only ${Blockly.nodeOutputs} outputs available");`,
       `    return false;`,
       `  }`,
       `  else {`,
       `    return true;`,
...
Blockly.JavaScript['node_status'] = function(block) {
    ...
    // Get the input string
    var textValue = Blockly.JavaScript.valueToCode(block, 'TEXT_INPUT', Blockly.JavaScript.ORDER_ATOMIC);
